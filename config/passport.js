...
async (accessToken, refreshToken, profile, done) => {
  try {
    ...
    if (rows.length > 0) {
      const { rows: updated } = await query(`
        UPDATE users SET
          ...
          name=$3, avatar=$4, last_login=NOW()
    ...
    const { rows: newUser } = await query(`
      INSERT INTO users (id, google_id, email, name, avatar, access_token, refresh_token)
      VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *
    `, [googleId, googleId, email, name, avatar, accessToken, refreshToken || '']);
